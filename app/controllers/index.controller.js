...
indexController.get = async (req, res) => {
	try {
		...
		const url = 'http://localhost:3000/api/date/get';
...
indexController.verify = async (req, res) => {
	try {
		// Defines url for id
		const urlId = `http://localhost:3333/api/id/verify/${req.body.id}`;
		// Defines url for age
		const urlAge = `http://localhost:3030/api/age/verify/${req.body.age}`;
